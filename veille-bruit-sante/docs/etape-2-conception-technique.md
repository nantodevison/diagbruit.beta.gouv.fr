...
**Correctif (constaté au premier run réel, hors cadrage initial) — `domaine_sante`/`source_bruit` doivent être des `Literal`, pas des `string` libres :** le premier essai a montré le modèle produire des valeurs libres et descriptives (ex. `"bruit environnemental (transport, aérien, routier, ferroviaire)"`) plutôt que de piocher dans la liste fermée documentée à l'étape 1 — en plus de s'écarter du contrat de données, ces valeurs contiennent des virgules, qu'un `select`/`multi_select` Notion refuse purement et simplement à l'écriture (voir `etape-3-conception-technique.md`, Décision 3). L'implémentation Pydantic (`etape2_recherche_extraction/extraction.py`) contraint donc ces deux champs à `List[Literal[...]]`, avec les mêmes valeurs que celles déclarées dans `etape1_base_notion/creer_base_notion.py` (`OPTIONS_DOMAINE_SANTE`/`OPTIONS_SOURCE_BRUIT`, importées plutôt que dupliquées) : les sorties structurées garantissent alors que le modèle ne peut renvoyer qu'une valeur de la liste fermée, par construction — pas seulement par consigne de prompt.
...
**Point d'attention — `parsed_output` peut être vide :** `client.messages.parse()` ne garantit pas que `reponse.parsed_output` soit toujours renseigné (rencontré au premier run réel, sans qu'une exception ne soit levée) — le code vérifie explicitement ce cas et lève une erreur exploitée par la Décision 6 (échec isolé, étude écartée) plutôt que de laisser remonter un `AttributeError` sur `None`.
