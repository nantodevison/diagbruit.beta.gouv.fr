...
5. **Export de la géométrie** : écrit `etape6_{dept}_geometries/{id_geometrie}.geojson` (une géométrie, en EPSG:4326, comme `etape5_{dept}.gpkg`). Seule propriété portée par le GeoJSON : `id_geometrie` — **délibérément pas `alert_slug`**, voir `etape-7-conception-technique.md`, "⚠️ Correctif requis avant le prochain dépôt" : à ce stade de l'étape 6, `alert_slug_propose` n'est le plus souvent qu'une proposition mécanique incomplète (voir "Reprise à la régénération" ci-dessous), la valeur finale n'étant connue qu'après la complétion manuelle en Phase 2 — la propriété `alert_slug` est ajoutée au fichier local juste avant son dépôt sur Box, à l'étape 7, pas ici.
