...
3. **Proposition de `label_propose`** (voir "Écart avec le schéma Strapi réel") : `ZONE CALME` si `nature_sonore_zone == "preservation_zone_calme"`, `ZONE SOUMISE AU BRUIT` sinon (y compris vide, pour `rnu`/`document_non_significatif`/`trou_de_couverture`).
