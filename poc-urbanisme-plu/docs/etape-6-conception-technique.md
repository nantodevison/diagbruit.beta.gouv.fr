...
*Document de cadrage technique, faisant suite à `etape-6-mise-en-forme-diagbruit.md` et à `etape-5-conception-technique.md`. Version issue des échanges du 21/08/2026, complétée le 24/08/2026 (champs granulaires Strapi/Notion, en préparant `etape-7-stockage-diagbruit.md`).*
...
└── output/
    ...
    ├── etape5_{dept}.gpkg                     # dont `titre_propose`, ajouté le 24/08/2026 (voir plus bas)
...
Lit `etape5_{dept}.gpkg` (couche `messages`), `etape5_{dept}_documents_par_synthese.csv`, `etape5_{dept}_occurrences.csv` et `etape4_{dept}.gpkg` (couche `geometries`, pour `communes` et `nature_sonore_zone` — voir écart constaté plus bas). Pour chaque géométrie finale :

1. **Assemblage des champs Strapi** (`assembler_message.py`) : à partir de `message_synthese` et de la liste des `nature`/`reference_precise` issues de `etape5_{dept}_documents_par_synthese.csv` filtrée sur `id_geometrie_synthese == id_geometrie` — le bloc `message_strapi` complet (formalisme de `etape-6-mise-en-forme-diagbruit.md`, "Assemblage du message pour Strapi"), **et**, depuis le 24/08/2026, les mêmes valeurs exposées séparément (`message_content`, `strapi_source`, `strapi_reference` — voir "Écart avec le schéma Strapi réel" ci-dessous). **Avec déduplication** (voir "Pourquoi la déduplication est correcte" ci-dessous) : `nature`/`reference_precise` sont dédupliqués avant jointure.
...
3. **Proposition de `label_propose`** (ajouté le 24/08/2026, voir "Écart avec le schéma Strapi réel") : `ZONE CALME` si `nature_sonore_zone == "preservation_zone_calme"`, `ZONE SOUMISE AU BRUIT` sinon (y compris vide, pour `rnu`/`document_non_significatif`/`trou_de_couverture`).
4. **Reprise de `titre_propose`** — repris tel quel de `etape5_{dept}.gpkg` (généré par LLM et validé à l'étape 5, voir `docs/etape-5-ameliorations-possibles.md`, "Génération LLM d'un titre court"), sans transformation. Remplace, depuis le 24/08/2026, le calcul précédent de `notion_description` par citations — voir `etape-7-stockage-diagbruit.md`, "Colonne Description".
...
## Pourquoi la déduplication est correcte (tranché le 24/08/2026, après une hésitation le même jour)

**Lire avant de retirer ce mécanisme.** `strapi_source` et `strapi_reference` sont dédupliqués (`dedupliquer_en_conservant_ordre`, dans `assembler_message.py`) avant jointure. Ce n'est pas un nettoyage arbitraire de la donnée : c'est la conséquence directe d'une décision humaine déjà prise et validée à l'étape 5. **Mise à jour du 24/08/2026** : `notion_description` (calculé par citations, lui aussi dédupliqué) n'existe plus — remplacé par `titre_propose`, le raisonnement ci-dessous ne s'applique donc plus qu'à `strapi_source`/`strapi_reference`.
...
**Erreur commise puis corrigée le même jour** : une version intermédiaire avait retiré cette déduplication, sur un raisonnement qui s'arrêtait à l'étape 1 ci-dessus ("des valeurs identiques ne veulent pas dire des occurrences identiques", vrai mais incomplet) sans aller jusqu'à l'étape 2-3 (la fusion déjà validée rend cette distinction non pertinente pour un champ de synthèse). Sans déduplication, `strapi_reference` aurait affiché `"Article 4 / Article 4 / Article 4 / Article 4"` — jamais produit par une saisie manuelle réelle (vérifié sur une capture d'écran Strapi).
...
## Écart avec le schéma Strapi réel (constaté le 23-24/08/2026)
...
**Mise à jour du 24/08/2026** : `title` (schéma Strapi) et `Description` (base Notion) sont désormais couverts par ce contrat, tous deux alimentés par `titre_propose` (voir ci-dessus et `docs/etape-5-ameliorations-possibles.md`, "Génération LLM d'un titre court") — `title` n'était auparavant pas renseigné (saisi manuellement à la relecture du brouillon Strapi) et `Description` calculée par citations.

## Reprise à la régénération (ajouté le 24/08/2026)
...
- `territoire_propose`, `label_propose` (ajouté le 24/08/2026, en badge à côté du territoire), `nom_fichier_geometrie` et `communes` en lecture seule, pour contexte ;
...
| `strapi_source` | liste dédupliquée des `nature` (`etape5_{dept}_documents_par_synthese.csv`), jointe par `" / "` (voir "Pourquoi la déduplication est correcte") — correspond au champ Strapi `source`. |
