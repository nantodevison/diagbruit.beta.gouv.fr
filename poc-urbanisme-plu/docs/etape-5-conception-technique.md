...
Chaque message généré (`message_occurrence`, `message_synthese_llm`, et depuis le 24/08/2026 `titre_propose_llm`) est **immuable** une fois écrit par la Phase 2 : jamais réécrit en place. À côté de chacun, trois colonnes, vides à la sortie de la Phase 2, remplies le cas échéant par l'opérateur en Phase 3 :
...
Pour chaque géométrie, résout la valeur finale : `message_synthese_corrige` si `synthese_corrigee`, sinon `message_synthese_llm` — et, indépendamment, `titre_propose_corrige` si `titre_corrigee`, sinon `titre_propose_llm` (ajouté le 24/08/2026, même résolution que le message). Écrit `etape5_{dept}.gpkg` — couche unique `messages`, contrat pour l'étape 6 — avec `message_synthese`/`titre_propose` (les valeurs résolues) **et** le détail complet (`message_synthese_llm`, `synthese_corrigee`, `message_synthese_corrige`, `titre_propose_llm`, `titre_corrigee`, `titre_propose_corrige`, `validation_message_commentaire`) conservé pour traçabilité, plutôt que seulement les valeurs finales.
...
| `titre_propose_llm` | ajouté le 24/08/2026 (voir "Phase 2" ci-dessus) — rédigé par le LLM à partir de `message_synthese_llm` (ou titre fixe pour `rnu`/`document_non_significatif`/`trou_de_couverture`). Immuable. |
