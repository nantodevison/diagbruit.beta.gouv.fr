...
**Besoin** : plusieurs occurrences peuvent, une fois localisées, s'avérer porter sur le même secteur et le même objectif de lutte contre le bruit (`nature_sonore_zone`), tout en décrivant chacune une règle réellement distincte (ex. une prescription du règlement écrit et une recommandation de l'OAP d'un même PLUi, toutes deux réelles mais formulées séparément). Sans mécanisme dédié, chacune produirait sa propre géométrie et, en aval, son propre message — redondant pour l'utilisateur final de diagBruit, alors que le contenu des deux mérite d'être conservé.
