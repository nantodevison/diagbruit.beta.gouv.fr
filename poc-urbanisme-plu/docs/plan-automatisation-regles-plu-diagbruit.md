...
4. **Création des géométries**, en deux phases :
   ...
   - **Volet B — documents significatifs** : la géométrie dépend de la portée de chaque occurrence validée (champ `portee_geometrique`, déterminé à l'étape 2 et corrigeable à l'étape 3).
     ...
     - *Alinéa 2* — portée `zone_specifique` : tracé manuel dans un outil SIG (QGIS). Révisé le 09/09/2026 : une correspondance automatique dans la couche de zonage officielle du GPU (`zone-urba`) est désormais tentée en premier, à partir du code de zone cité dans le texte — succès vérifié en conditions réelles sur l'Eurométropole de Strasbourg. Le tracé manuel reste le repli quand aucun code de zone exploitable n'a pu être extrait du texte (cas fréquent pour les occurrences issues d'une OAP, qui nomment un secteur par un lieu plutôt que par un code de zonage) ou quand le code cité ne correspond à aucune zone numérisée dans le GPU pour ce document — voir `etape-4-construction-geometries-diagbruit.md`, "Sources de géométrie".
...
## Doublon vs fusion : deux notions à ne pas confondre

*Ajouté le 29/08/2026, suite à un constat en relecture (étape 3) sur le 067 hors Eurométropole : plusieurs occurrences manuelles à vérifier concernaient en réalité le même texte. Deux mécanismes du plan se ressemblent en surface (tous deux relient deux occurrences entre elles) mais répondent à des besoins opposés — la confusion entre les deux est le principal risque à éviter, d'où cette section dédiée plutôt qu'une simple note dans la doc de l'étape concernée.*

- **Doublon** — une **erreur de détection** : deux occurrences décrivent en réalité **la même action, du même document, sur la même zone, et de la même nature sonore** (zone soumise au bruit ou zone calme). Ce n'est pas une deuxième règle : c'est la même règle, repérée deux fois par l'étape 2 (par exemple parce que le règlement écrit et l'OAP d'un même PLUi la reformulent chacun à leur façon). Une seule des deux occurrences doit survivre dans la couverture SIG finale ; l'autre est un artefact de détection à écarter — traité **à l'étape 3**, avant qu'une géométrie n'existe pour l'une ou l'autre (voir `etape-3-validation-manuelle.md`, "Détection et traitement des doublons").
- **Fusion** — **pas une erreur de détection** : deux occurrences décrivent **deux actions différentes**, mais sur la même zone et de la même nature sonore, que l'on choisit de regrouper pour que diagBruit les affiche **ensemble** dans ses recommandations plutôt qu'en deux messages séparés pour le même endroit. Les deux règles sont réelles et toutes deux conservées ; seul leur affichage est mutualisé. Traité **à l'étape 4**, au moment du tracé géométrique manuel (voir `etape-4-conception-technique.md`, "Mécanisme de fusion").

Conséquence directe sur l'articulation des étapes : parce que les doublons sont désormais résolus dès l'étape 3, une occurrence qui atteint l'étape 4 (`etape3_{dept}.csv`) porte forcément un contenu réel et distinct des autres — le mécanisme de fusion de l'étape 4 n'a donc plus à se demander si un rapprochement qu'on lui propose est une erreur de détection à corriger : il ne traite que de vraies actions à regrouper pour l'affichage.
