...
| `type_piece_source`, `reference_type`, `reference_precise`, `extrait_significatif`, `contexte_documentaire`, `confiance_extrait`, `ocr_utilise`, `ocr_confiance` | pour `occurrence_locale`, valeur reprise telle quelle de l'étape 2 — aucun champ de formulaire ne permet à l'opérateur de les modifier dans `outil_validation.html` (voir Phase 2). Vide sur une ligne de synthèse "document non significatif", "document non exploitable" ou "trou de couverture". |
