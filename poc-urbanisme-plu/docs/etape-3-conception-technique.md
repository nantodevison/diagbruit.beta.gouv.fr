...
**Exclusion des paires à zone réglementaire différente (ajoutée le 09/09/2026, récupération automatique des zones PLU à l'étape 4)** : une paire n'est comparée que si `zone_reglementaire_mentionnee` est identique des deux côtés, ou vide d'au moins un côté. Nécessaire depuis que l'étape 2 éclate une occurrence citant plusieurs zones ("UA, UB et N") en plusieurs lignes distinctes, une par zone, partageant le même `extrait_significatif` par construction (voir `etape-2-conception-technique.md` et `etape2_analyse_reglements/synthese.py`, `_eclater_par_zone`) — sans cette exclusion, ces lignes sœurs auraient systématiquement été suggérées comme doublons probables l'une de l'autre alors que ce sont des occurrences distinctes à conserver, chacune destinée à sa propre géométrie.
...
| `zone_reglementaire_mentionnee`, `justification`, `nature_occurrence`, `nature_juridique_piece`, `nature_sonore_zone` | pour `occurrence_locale`, valeur finale — potentiellement corrigée par l'opérateur, ce sont de véritables champs de formulaire dans `outil_validation.html` (voir Phase 2). Vide sur une ligne de synthèse "document non significatif", "document non exploitable" ou "trou de couverture". **Exception : `justification`, pré-remplie pour RNU** (texte fixe citant l'article R.111-2 du code de l'urbanisme). **`zone_reglementaire_mentionnee` (mis à jour le 09/09/2026)** : toujours une **seule** zone par ligne, jamais une liste — une occurrence citant plusieurs zones a déjà été éclatée en plusieurs lignes distinctes à l'étape 2 (voir `etape-2-conception-technique.md`). Obligatoire (bloqué par l'outil de relecture, voir `etape-3-validation-manuelle.md`, "Édition en une seule passe") quand `portee_geometrique = "zone_specifique"`, puisque l'étape 4 s'en sert pour une recherche automatique de géométrie. |
