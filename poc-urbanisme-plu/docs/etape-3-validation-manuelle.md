...
**`zone_reglementaire_mentionnee` obligatoire pour `zone_specifique` (ajouté le 09/09/2026, récupération automatique des zones PLU à l'étape 4)** : le bouton "✓ Valider" refuse désormais de faire passer une occurrence à `validé`/`corrigé` si `portee_geometrique = "zone_specifique"` et que `zone_reglementaire_mentionnee` est vide (toast d'avertissement, aucun changement d'état — même mécanisme que le blocage déjà en place sur le bouton "◎ Doublon"). Nécessaire car l'étape 4 tente désormais une recherche automatique de la géométrie de la zone (couche `zone-urba` du GPU) à partir de ce champ avant de retomber sur le tracé manuel — un champ vide condamnerait d'office cette recherche, ce qui n'était pas grave tant que `zone_reglementaire_mentionnee` restait purement informatif, mais le devient une fois qu'il pilote une recherche automatique. Contrat par ailleurs inchangé pour l'opérateur : une seule zone par occurrence, jamais une liste — une règle qui cite plusieurs zones ("UA, UB et N") arrive déjà éclatée en plusieurs occurrences distinctes depuis l'étape 2 (voir `etape2_analyse_reglements/synthese.py`, `_eclater_par_zone`), chacune avec sa propre `zone_reglementaire_mentionnee`, jamais à recomposer manuellement ici.
