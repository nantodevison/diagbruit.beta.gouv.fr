...
| `numero_page` | Entier | `filtrage_lexical.py` (phase 3), via `_ParagrapheRepere.numero_page`. Toujours renseigné, y compris quand `reference_type = "alinea"` (ajouté le 28/08/2026 — avant cette date, le numéro de page n'était transporté jusqu'à `PassageRetenu` qu'en repli `reference_type = "page"`, jamais aux côtés d'une référence d'article/alinéa). Sert de repère de recherche dans le PDF source à la relecture (étape 3), voir `etape-3-conception-technique.md`. |
