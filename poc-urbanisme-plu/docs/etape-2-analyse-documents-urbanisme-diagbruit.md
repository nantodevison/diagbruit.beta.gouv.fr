...
- la ou les **zones réglementaires** concernées, quand le texte les précise (une par zone — ex. "UA", "UB", "N" — recopiée telle qu'écrite dans le texte ; texte libre non codifié type "ensemble du zonage" ou "non précisé" si le texte ne cite aucun code de zone) ; **mis à jour le 09/09/2026** : une règle qui cite plusieurs zones ("UA, UB et N") produit désormais plusieurs occurrences distinctes, une par zone, plutôt qu'une seule occurrence à zones multiples — voir Phase 5 ci-dessous ;
- la **portée géométrique** de la règle : **administrative** (la règle s'applique à l'ensemble du zonage, de la commune ou de l'EPCI couvert par le document — une géométrie de contour administratif suffira à l'étape 4) ou **zone spécifique** (la règle ne concerne qu'une zone réglementaire précise — ex. "UA"). Depuis le 09/09/2026, l'étape 4 tente d'abord une correspondance automatique de cette zone dans les données de zonage officielles du GPU avant de retomber sur un tracé manuel en l'absence de correspondance — voir `etape-4-construction-geometries-diagbruit.md`, "Sources de géométrie". Ce champ est distinct de la zone réglementaire mentionnée elle-même : l'un dit *quelle(s)* zone(s) sont citées dans le texte (`zone_reglementaire_mentionnee`, texte libre, informatif — et désormais le point d'entrée de cette recherche automatique), l'autre dit *quel processus de géométrie* appliquer (`portee_geometrique`, valeur contrôlée, exploitée directement par le code de l'étape 4) ;
...
| `id_occurrence` | identifiant unique de la ligne, construit à partir du nom du fichier source et d'un compteur qui repart à 1 pour chaque pièce. Une règle citant plusieurs zones réglementaires (voir `zone_reglementaire_mentionnee` ci-dessous) donne plusieurs `id_occurrence` consécutifs, un par zone |
...
| `zone_reglementaire_mentionnee` | une seule zone par ligne, jamais une liste (ex. "UA") ; une règle qui en cite plusieurs a déjà été éclatée en plusieurs lignes, voir `id_occurrence` ci-dessus ; "ensemble du zonage" ou "non précisé" en texte libre si le texte ne cite aucun code de zone |
