...
**Contexte** : `sources_gpu.trouver_geometrie_zone` (ajouté le 09/09/2026, voir `etape-4-construction-geometries-diagbruit.md`, "Sources de géométrie") compare `zone_reglementaire_mentionnee` au `libelle` de chaque zone de la partition après une normalisation minimale (espaces retirés, casse uniforme).

**Problème 1 — ambiguïté chiffre/romain** : les phases d'urbanisation future (AU) sont couramment écrites avec un chiffre dans le règlement ("1AUh", "2AU"...) mais numérisées avec un chiffre romain dans la couche `zone-urba` du GPU ("IAUB", "IIAU"...) — une convention cartographique différente de la convention rédactionnelle, pas une erreur de saisie d'un côté ou de l'autre. Constaté en explorant des données réelles (département 067 hors Eurométropole et Eurométropole de Strasbourg).

**Problème 2 — préfixe "secteur"/"zone" non retiré (constaté le 21/09/2026, retour utilisateur pendant le tracé manuel du département 067 hors Eurométropole)** : `zone_reglementaire_mentionnee` porte couramment un préfixe descriptif avant le code lui-même — ex. "Secteur Uh", "Zone N" — que ce soit produit par l'étape 2 ou saisi à la main par un opérateur en éclatant une occurrence multi-zones à l'étape 3 (voir la conversation du 11/09/2026 sur le département 067 : le cluster `1_67252_reglement_20250623.pdf` porte "Secteur Uh", "Secteur Uep", "Secteur Uj"...). Le `libelle` de la couche `zone-urba`, lui, n'est jamais préfixé ainsi (vérifié en réel : "N1", "UCA2", "UB1"...). Un opérateur a constaté en tracant manuellement plusieurs de ces occurrences que la zone existait bel et bien dans `zone-urba` sous son code nu — la correspondance aurait donc dû réussir, et n'a échoué qu'à cause de ce préfixe non retiré.

**Impact des deux** : pas une perte de donnée — une zone non trouvée retombe simplement sur le tracé manuel intégral, exactement comme si elle n'avait jamais été tentée automatiquement (voir "Correspondance automatique de zone (`zone-urba`)" dans `etape-4-conception-technique.md`). Le coût est un taux de correspondance automatique plus faible que le maximum atteignable — mesuré comme réel et non négligeable pour le problème 2 (au moins un cluster de 6 zones entièrement manqué sur le seul département 067 hors Eurométropole).

**Piste de correction, retenue pour le problème 2, pas encore implémentée** : ajouter, dans `sources_gpu._normaliser_code_zone`, le retrait d'un préfixe `secteur(s)`/`zone(s)` en tête de chaîne (éventuellement précédé de "la"/"le"/"les"/"du"/"de la"), insensible à la casse, avant la comparaison — jamais sur la valeur stockée/affichée de `zone_reglementaire_mentionnee`, seulement sur la comparaison interne. Volontairement pas encore codé au moment de cette note : le département 067 hors Eurométropole est en cours de tracé manuel (Phase 2, QGIS) au moment de la découverte, et le correctif vit dans le module utilisé par `preparer_geometries.py` (Phase 1) — pour ne pas risquer d'interférer avec ce travail en cours, même si le garde-fou "refus si le fichier de sortie existe déjà" (voir plus bas, section retirée de ce document une fois implémentée) empêche de toute façon une régénération accidentelle. À coder une fois le tracé manuel de ce département terminé.
