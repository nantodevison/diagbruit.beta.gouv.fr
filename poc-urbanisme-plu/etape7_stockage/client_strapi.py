...
class ConfigurationStrapiManquante(Exception):
    """`STRAPI_API_TOKEN` ou `STRAPI_URL` absent de l'environnement."""
...
def _base_url() -> str:
    ...
    if not url:
        raise ConfigurationStrapiManquante(
            "STRAPI_URL n'est pas définie. Voir docs/etape-7-conception-technique.md."
        )
    return url.rstrip("/")


def _jeton() -> str:
    jeton = os.environ.get("STRAPI_API_TOKEN")
    if not jeton:
        raise ConfigurationStrapiManquante(
            "STRAPI_API_TOKEN n'est pas définie. Voir docs/etape-7-conception-technique.md."
        )
    return jeton
...
def _post(url: str, payload: dict) -> dict:
    ...
    response = requests.post(
        url, headers={**_headers(), "Content-Type": "application/json"}, params={"status": "draft"}, json=payload, timeout=15
    )
    response.raise_for_status()
    return response.json()
...
def _chercher(alert_slug: str, status: str) -> str | None:
    donnees = _get(
        f"{_base_url()}/api/{CONTENT_TYPE}", params={"filters[alert_slug][$eq]": alert_slug, "status": status}
    )
    resultats = donnees.get("data") or []
    return resultats[0]["documentId"] if resultats else None


def trouver_document_id(alert_slug: str) -> str | None:
    ...
    **Piège corrigé le 24/08/2026, après avoir créé 19 doublons en réel** :
    ...
    """
    return _chercher(alert_slug, "draft") or _chercher(alert_slug, "published")
...
def creer_ou_mettre_a_jour(champs: ChampsNoisezoneAlert) -> tuple[str, bool]:
    ...
    document_id = trouver_document_id(champs.alert_slug)
    url_collection = f"{_base_url()}/api/{CONTENT_TYPE}"
    if document_id:
        _put(f"{url_collection}/{document_id}", _payload(champs))
        return document_id, False
    reponse = _post(url_collection, _payload(champs))
